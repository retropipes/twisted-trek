...
class Floor1 extends FuncFloor {
	...
	if (vb.opb.read(1) == 1) {
		...
		final int predicted = Floor1.render_point(info.postlist[look.loneighbor[i - 2]],
    ...
    private static int render_point(final int x0, final int x1, final int y0_in, final int y1_in, final int x) {
	int y0 = y0_in;
	int y1 = y1_in;
    ...
    class InfoFloor1 {
	static final int LOCAL_VIF_POSIT = 63;
	...
	int mult; /* 1 2 3 or 4 */
	int[] postlist = new int[InfoFloor1.LOCAL_VIF_POSIT + 2]; /* first two implicit */
	...
	Object copy_info() {
	    ...
	    ret.mult = info.mult;
	    System.arraycopy(info.postlist, 0, ret.postlist, 0, InfoFloor1.LOCAL_VIF_POSIT + 2);
	...
	static final int LOCAL_VIF_POSIT = 63;
	int[] sorted_index = new int[LookFloor1.LOCAL_VIF_POSIT + 2];
	int[] forward_index = new int[LookFloor1.LOCAL_VIF_POSIT + 2];
	int[] reverse_index = new int[LookFloor1.LOCAL_VIF_POSIT + 2];
	int[] hineighbor = new int[LookFloor1.LOCAL_VIF_POSIT];
	int[] loneighbor = new int[LookFloor1.LOCAL_VIF_POSIT];
	int posts;
	int n;
	int quant_q;
	InfoFloor1 vi;
