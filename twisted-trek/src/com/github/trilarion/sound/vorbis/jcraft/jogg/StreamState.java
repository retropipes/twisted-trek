...
public class StreamState {
    ...
    StreamState(final int new_serialno) {
	this();
	this.init(new_serialno);
    }
     ...
     * @param new_serialno
     */
    public final void init(final int new_serialno) {
	...
	final int local_header = og.header;
	...
	final long local_granulepos = og.granulepos();
	final int _serialno = og.serialno();
	final int _pageno = og.pageno();
	final int segments = header_base[local_header + 26] & 0xff;
	...
	if (_pageno != this.pageno) {
		...
		for (; segptr < segments; segptr++) {
		    final int val = header_base[local_header + 27 + segptr] & 0xff;
		    body += val;
		    bodysize -= val;
		    if (val < 255) {
			segptr++;
	...
	}
	{
	    ...
	    if (saved != -1) {
		this.granule_vals[saved] = local_granulepos;
	    }
	}
	if (eos != 0) {
	    this.e_o_s = 1;
