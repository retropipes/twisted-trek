...
public class VorbisFile {
    ...
    private int get_next_page(final Page page, final long boundary_in) {
	long boundary = boundary_in;
	if (boundary > 0) {
    ...
    int bisect_forward_serialno(final long begin, final long searched_in, final long new_end, final int currentno,
	    final int m) {
	long searched = searched_in;
	long endsearched = new_end;
	long next = new_end;
	...
	if (searched >= new_end || ret == -1) {
	    this.links = m + 1;
	    this.offsets = new long[m + 2];
	    this.offsets[m + 1] = searched;
	} else {
	    ret = this.bisect_forward_serialno(next, this.offset, new_end, page.serialno(), m + 1);
	    if (ret == VorbisFile.OV_EREAD) {
		return VorbisFile.OV_EREAD;
	    }
	}
    ...
    int fetch_headers(final Info new_vi, final Comment new_vc, final int[] serialno, final Page og_ptr) {
	Page new_og_ptr = og_ptr;
	...
	if (serialno != null) {
	    serialno[0] = new_og_ptr.serialno();
	}
	this.os.init(new_og_ptr.serialno());
	...
	while (i < 3) {
		...
		if (new_vi.synthesis_headerin(new_vc, op) != 0) {
		    new_vi.clear();
		    new_vc.clear();
		    this.os.clear();
		    return -1;
	    ...
	    if (i < 3) {
		if (this.get_next_page(new_og_ptr, 1) < 0) {
		    new_vi.clear();
		    new_vc.clear();
		    this.os.clear();
		    return -1;
	...
	for (int i = 0; i < this.links; i++) {
	    ...
	    {
		final long local_end = this.offsets[i + 1]; // !!!
		this.seek_helper(local_end);
		while (true) {
	...
	local_end = this.offset;
	// We get the offset for the last page of the physical bitstream.
	// Most OggVorbis files will contain a single logical bitstream
	local_end = this.get_prev_page(og);
	// moer than one logical bitstream?
	if (og.serialno() != serialno) {
	    // Chained bitstream. Bisect-search each logical bitstream
	    // section. Do so based on serial number only
	    if (this.bisect_forward_serialno(0, 0, local_end + 1, serialno, 0) < 0) {
		this.clear();
		return VorbisFile.OV_EREAD;
	    }
	} else {
	    // Only one logical bitstream
	    if (this.bisect_forward_serialno(0, local_end, local_end + 1, serialno, 0) < 0) {
		this.clear();
		return VorbisFile.OV_EREAD;
	    }
	}
	...
	// would be an error condition)
	{
	    ...
	    long local_end = this.offsets[link + 1];
		...
		long bisect;
		int ret;
		if (local_end - begin < VorbisFile.CHUNKSIZE) {
		    bisect = begin;
		} else {
		    bisect = (local_end + begin) / 2;
		}
		this.seek_helper(bisect);
		ret = this.get_next_page(og, local_end - bisect);
		if (ret == -1) {
		    local_end = bisect;
		} else {
    ...
    int read(final byte[] buffer, final int length, final int bigendianp, final int word, final int sgned,
	    final int[] bitstream) {
	final int host_endian = VorbisFile.host_is_big_endian();
	int index = 0;
	while (true) {
	    if (this.decode_ready) {
		float[][] pcm;
