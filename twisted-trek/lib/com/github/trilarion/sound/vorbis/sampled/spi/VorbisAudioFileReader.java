...
public class VorbisAudioFileReader extends TAudioFileReader {
     ...
     * @throws java.io.IOException
     */
    protected AudioFileFormat getAudioFileFormat(final InputStream bitStream, final int mediaLength,
	    final int totalms_in) throws UnsupportedAudioFileException, IOException {
	int totalms = totalms_in;
	...
	// AudioSystem.NOT_SPECIFIED, vorbisInfo.channels, AudioSystem.NOT_SPECIFIED,
	// AudioSystem.NOT_SPECIFIED, true,af_properties);
	...
	// New Patch from MS:
	final AudioFormat format = new AudioFormat(VorbisAudioFileReader.VORBISENC, this.vorbisInfo.rate,
     ...
     * @throws java.io.IOException
     */
    public AudioInputStream getAudioInputStream(final InputStream inputStream_in, final int medialength,
	    final int totalms) throws UnsupportedAudioFileException, IOException {
	InputStream inputStream = inputStream_in;
	try {
	    if (!inputStream.markSupported()) {
		inputStream = new BufferedInputStream(inputStream);
	    ...
	    inputStream.mark(VorbisAudioFileReader.MARK_LIMIT);
	    final AudioFileFormat audioFileFormat = this.getAudioFileFormat(inputStream, medialength, totalms);
	    inputStream.reset();
     ...
     */
    private void readHeaders(final Map<String, Object> aff_properties, final Map<String, Object> af_properties)
	    throws IOException {
	...
	for (final byte[] ptr1 : ptr) {
	    ...
	    final String currComment = new String(ptr1, 0, ptr1.length - 1, "UTF-8").trim();
     ...
     */
    private int readFromStream(final byte[] new_buffer, final int new_index, final int bufferSize_) {
	int local_bytes;
	try {
	    local_bytes = this.oggBitStream_.read(new_buffer, new_index, bufferSize_);
