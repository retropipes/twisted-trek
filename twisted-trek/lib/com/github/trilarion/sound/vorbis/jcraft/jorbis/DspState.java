...
public class DspState {
	...
	case 0: // The 'vorbis window' (window 0) is sin(sin(x)*sin(x)*2pi)
	{
		...
		float x = (float) ((right - i - .5) / right * DspState.M_PI / 2.);
		x = (float) Math.sin(x);
		x *= x;
		x *= DspState.M_PI / 2.;
		x = (float) Math.sin(x);
    ...
    final int init(final Info new_vi, final boolean encp) {
	this.vi = new_vi;
	this.modebits = Util.ilog2(new_vi.modes);
	...
	((Mdct) this.transform[0][0]).init(new_vi.blocksizes[0]);
	((Mdct) this.transform[1][0]).init(new_vi.blocksizes[1]);
	...
	for (int i = 0; i < DspState.VI_WINDOWB; i++) {
	    this.window[0][0][0][i] = DspState.window(i, new_vi.blocksizes[0], new_vi.blocksizes[0] / 2,
		    new_vi.blocksizes[0] / 2);
	    this.window[1][0][0][i] = DspState.window(i, new_vi.blocksizes[1], new_vi.blocksizes[0] / 2,
		    new_vi.blocksizes[0] / 2);
	    this.window[1][0][1][i] = DspState.window(i, new_vi.blocksizes[1], new_vi.blocksizes[0] / 2,
		    new_vi.blocksizes[1] / 2);
	    this.window[1][1][0][i] = DspState.window(i, new_vi.blocksizes[1], new_vi.blocksizes[1] / 2,
		    new_vi.blocksizes[0] / 2);
	    this.window[1][1][1][i] = DspState.window(i, new_vi.blocksizes[1], new_vi.blocksizes[1] / 2,
		    new_vi.blocksizes[1] / 2);
	}
	this.fullbooks = new CodeBook[new_vi.books];
	for (int i = 0; i < new_vi.books; i++) {
	    this.fullbooks[i] = new CodeBook();
	    this.fullbooks[i].init_decode(new_vi.book_param[i]);
	}
	...
	this.pcm = new float[new_vi.channels][];
	{
	    for (int i = 0; i < new_vi.channels; i++) {
	...
	this.centerW = new_vi.blocksizes[1] / 2;
	this.pcm_current = this.centerW;
	// initialize all the mapping/backend lookups
	this.mode = new Object[new_vi.modes];
	for (int i = 0; i < new_vi.modes; i++) {
	    final int mapnum = new_vi.mode_param[i].mapping;
	    final int maptype = new_vi.map_type[mapnum];
	    this.mode[i] = FuncMapping.mapping_P[maptype].look(this, new_vi.mode_param[i], new_vi.map_param[mapnum]);
	}
	return 0;
    }
     ...
     * @return
     */
    public int synthesis_init(final Info new_vi) {
	this.init(new_vi, false);
	// Adjust centerW to allow an easier mechanism for determining output
	this.pcm_returned = this.centerW;
	this.centerW -= new_vi.blocksizes[this.W] / 4 + new_vi.blocksizes[this.lW] / 4;
	this.granulepos = -1;
	this.sequence = -1;
	return 0;
    }

    DspState(final Info new_vi) {
	this();
	this.init(new_vi, false);
	// Adjust centerW to allow an easier mechanism for determining output
	this.pcm_returned = this.centerW;
