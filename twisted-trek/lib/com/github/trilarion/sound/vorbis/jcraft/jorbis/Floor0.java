...
class Floor0 extends FuncFloor {
	...
	for (int j = 0; j < look.n; j++) {
	    int val = (int) Math.floor(Floor0.toBARK((float) (info.rate / 2. / look.n * j)) * scale); // bark numbers
    ...
    static Object state(final Object i) {
	...
	final LookFloor0 look = (LookFloor0) i;
	final InfoFloor0 info = look.vi;
	float[] local_lsp = null;
	if (memo instanceof float[]) {
	    local_lsp = (float[]) memo;
	}
	final int ampraw = vb.opb.read(info.ampbits);
	if (ampraw > 0) { // also handles the -1 out of data case
	    final int maxval = (1 << info.ampbits) - 1;
		...
		final CodeBook b = vb.vd.fullbooks[info.books[booknum]];
		float last = 0.f;
		if (local_lsp == null || local_lsp.length < look.m + 1) {
		    local_lsp = new float[look.m + 1];
		} else {
		    for (int j = 0; j < local_lsp.length; j++) {
			local_lsp[j] = 0.f;
		    }
		}
		for (int j = 0; j < look.m; j += b.dim) {
		    if (b.decodev_set(local_lsp, j, vb.opb, b.dim) == -1) {
			return null;
		    }
		}
		for (int j = 0; j < look.m;) {
	...
	final InfoFloor0 info = look.vi;
	if (memo != null) {
	    final float[] local_lsp = (float[]) memo;
	    final float amp = local_lsp[look.m];
	    Lsp.lsp_to_curve(out, look.linearmap, look.n, look.ln, local_lsp, look.m, amp, info.ampdB);
	    return 1;
	}
	for (int j = 0; j < look.n; j++) {
	    out[j] = 0.f;
